...
/**
 * Sponsor-a-newbie flow for confirmed alumni. Extracted from the old
 * ConfirmedMemberDetailsForm — the logistics fields it used to carry now
 * live on /profile.
 */
export function SponsorNewbieForm() {
  ...
  if (details === undefined || invites === undefined || config === undefined) {
    return <p className="mt-6 text-sm text-slate-400">Loading sponsorship details...</p>;
  }
  ...
  if (!canSponsorNewbies) {
    return null;
  }

  return (
    <div className="mt-6 rounded-lg bg-white/5 p-4 ring-1 ring-white/10 text-left">
