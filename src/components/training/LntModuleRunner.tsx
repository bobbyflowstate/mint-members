...
export function LntModuleRunner({ memberName, initialState, completedAt, onSave, onComplete, onDone }: Props) {
  ...
  if (state.step === 0) {
    content = <div className="flex min-h-[520px] flex-col justify-center">
      ...
      <p className="mt-5 text-lg leading-7 text-slate-300">Eight minutes, once. Pack right, sort right, and don&apos;t leave the camp eating your mistake.</p>
    ...
    content = <>
      ...
      <div className="mt-3 space-y-3">
        {([
          ["camp", "Just camping", "The short version—about 8 minutes"],
    ...
    content = <><h2 className="text-3xl font-bold text-white">Twelve places anything can go</h2><p className="mt-3 text-slate-400">Open a stream to see what belongs there. You&apos;ll practise next.</p><div className="mt-6 grid grid-cols-3 gap-2">{lntModule.content.streams.map((stream) => <button key={stream.id} onClick={() => setSelectedStream(stream.id)} className="flex min-h-28 flex-col items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 p-2 text-center"><StreamBadge id={stream.id} compact /><span className="text-[11px] leading-tight text-slate-300">{stream.name}</span></button>)}</div></>;
    action = <button className="training-cta" onClick={() => goTo(6)}>Practise</button>;
    ...
    if (state.quizQueue.length === 0) {
      ...
      content = <><div className="mb-5 flex gap-2">{lntModule.content.quizItems.map((_, index) => <span key={index} className={`h-2 w-2 rounded-full ${state.quizMarks[index] === true ? "bg-emerald-400" : state.quizMarks[index] === false ? "bg-red-400" : "bg-white/15"}`} />)}</div><div className={`${card} py-7 text-center`}><div className="text-5xl">{quiz.icon}</div><h2 className="mt-3 text-2xl font-bold text-white">{quiz.name}</h2><p className="mt-2 text-sm text-slate-500">{quiz.scenario}</p></div>{answer ? <div className={`mt-4 rounded-2xl border p-4 ${answer.correct ? "border-emerald-400/40 bg-emerald-400/10" : "border-red-400/40 bg-red-400/10"}`}><h3 className="text-xl font-bold text-white">{answer.correct ? "Yep" : "Not that one"}</h3><p className="mt-2 text-sm leading-6 text-slate-300">{quiz.explanation}</p><p className="mt-2 text-xs text-slate-500">{quiz.source}</p></div> : <div className="mt-4 grid grid-cols-4 gap-2">{QUIZ_CHOICES.map((id) => { const stream = lntModule.content.streams.find((item) => item.id === id)!; return <button key={id} onClick={() => chooseQuizAnswer(id)} className="flex min-h-20 flex-col items-center justify-center gap-1 rounded-xl border border-white/10 bg-white/5 p-1" aria-label={stream.name}><StreamBadge id={id} compact /><span className="sr-only">{stream.name}</span></button>; })}</div>}</>;
